...
import { open, openAsDraft, setMode } from "redux/features/folderFormSlice";
...
export default function CreateFolderButton() {
  ...
  const create = () => {
    if (isDraft(draft, "folder")) {
      d(openAsDraft(getFormDraft(draft, "folder")));
    } else {
      d(setMode("create"));
      d(open());
    }
  ...
  return (
    <Button
      ...
      onClick={create}
    >
      {labels[language.code].createFolder}
    </Button>
  );
}
